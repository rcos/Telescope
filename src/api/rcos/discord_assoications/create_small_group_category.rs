...
#[graphql(
    ...
    query_path = "graphql/rcos/discord_assoications/create_small_group_category.graphql",
)]
pub struct CreateOneSmallGroupCategory;


impl CreateOneSmallGroupCategory{
    pub async fn execute(small_group_id : i64,  category_id: String) -> Result<Option<String>, TelescopeError>{
        send_query::<Self>(create_one_small_group_category::Variables{
            small_group_id,
            category_id,
        }).await
        .map(|response| response.insert_small_group_categories_one.map(|obj| obj.category_id))
