...
import api.db.users as users_db
...
async def list_users(
    ...
    return await users_db.fetch_users(db, filter_dict(locals(), ["is_rpi", "is_faculty", "timezone"]))
...
async def get_user(username: str, db: Connection = Depends(get_db)):
    user = await users_db.fetch_user(db, username)
    if user is None:
...
async def create_or_update_user(username: str, user: UserIn, db: Connection = Depends(get_db)):
    updated_user_dict = await users_db.upsert_user(db, username, user.dict(exclude_unset=True))
...
async def delete_user(username: str, db: Connection = Depends(get_db)):
    ...
    deleted_user = await users_db.delete_user(db, username)
...
async def get_user(username: str, db: Connection = Depends(get_db)):
    ...
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")
    return await users_db.fetch_user_accounts(db, username)
